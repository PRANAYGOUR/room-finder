      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          {room.images && room.images.length > 0 ? (
            ...
            <div className="rounded-md bg-indigo-800 h-72 md:h-96 flex items-center justify-center text-white">No photo</div>
          )}
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <p className="mt-2 text-indigo-200">{room.location}</p>
            ...
            <div className="mt-6">
              <div className="text-sm text-indigo-200">Rent</div>
              <div className="text-2xl font-bold text-purple-300">${room.rent}<span className="text-sm font-normal">/mo</span></div>
            </div>

            <div className="mt-6 text-indigo-100 space-y-2">
              <p><strong>Contact:</strong> <a href={`tel:${room.contact_number}`} className="text-white underline">{room.contact_number}</a></p>
            </div>

            <div className="mt-6">
              <p className="text-indigo-200">{room.description ?? ''}</p>
            </div>
          </div>

          <div className="mt-6 flex items-center gap-3">
            <a href={`tel:${room.contact_number}`} className="glow-cta px-4 py-2 rounded">Contact owner</a>
            <button className="px-4 py-2 rounded bg-white/8 text-white">Save</button>
          </div>
        </div>
      </div>
    </div>
